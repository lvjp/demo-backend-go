...
func Hash(password, salt []byte, params Params) *phc.String {
...
func NewParams(phc *phc.String) (*Params, error) {
	if phc.ID != ID {
		return nil, errors.New("unsupported hashing function: " + phc.ID)
	}

	if phc.Version != strconv.Itoa(argon2.Version) {
		return nil, errors.New("unsupported argon2id version: " + phc.Version)
	}

	// There are 3 parameters: m (memory), t (time), and p (parallelism).
	const parametersCount = 3

	if len(phc.Params) != parametersCount {
		return nil, errors.New("invalid parameter count: " + strconv.Itoa(len(phc.Params)))
	}

	if phc.Params[0].Name != "m" || phc.Params[1].Name != "t" || phc.Params[2].Name != "p" {
		return nil, errors.New("parameters should be in the order: m, t, p")
	}
	...
	if len(phc.Hash) > math.MaxUint32 {
		return nil, fmt.Errorf("hash is too long: %d", len(phc.Hash))
	}
